...
namespace BistMode.ViewModels
{
    public sealed class AutoRunVM : ViewModelBase
    {
        ...
        public void LoadFrom(JObject autoRunControl)
        {
            ...
            // PatOptions：顯示圖片名稱、Index 對應圖片牆數字
            PatternOptions.Clear();
            var optArray = autoRunControl["PatOptions"] as JArray;
            if (optArray != null)
            {
                Debug.WriteLine("[AutoRun] Parsing PatOptions...");
                foreach (var t2 in optArray.OfType<JObject>())
                {
                    ...
                    Debug.WriteLine($"[AutoRun] PatOption: Index={idx}, Name={name}");
                }
            }
            else
            {
                Debug.WriteLine("[AutoRun] PatOptions not found in JSON.");
            }
